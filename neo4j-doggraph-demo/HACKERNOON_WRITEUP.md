...
## An honest natural-language layer

We added a small natural-language → Cypher layer so you can ask the graph in plain English:

```bash
python ask.py "Is Kikoo's drift explained by context, or should I worry?"
```

**What it is, precisely:** a **deterministic intent router**. It maps a question to one of a handful of *audited, parameterised* Cypher queries and runs it. It does **not** use an LLM and it does **not** invent queries. For a health-adjacent demo, a fixed set of reviewed graph queries is safer and more honest than free-form generation.

The seam to upgrade to true LLM-backed, schema-constrained text-to-Cypher — i.e. real graph-grounded retrieval — is marked clearly in `graph_query.py`. Until that LLM step is wired in, we don't call it GraphRAG, because it isn't yet. Honesty about capability is the whole point of Barkley.
...
**Tags:** #proof-of-usefulness #neo4j #graph-database #knowledge-graph #machine-learning #synthetic-data #pet-tech
