...
> 🔗 **Try it live:** **[doggraph.getbarkley.com](https://doggraph.getbarkley.com/)** — the same code that lives in this folder, talking to a real AuraDB instance, with the LLM/GraphRAG layer enabled. Ask in natural language; the app shows the generated Cypher, the rows it returned, and the grounded answer. Nothing is mocked.
...
The same pipeline behind a small Streamlit UI: question → generated Cypher (always shown) → grounded answer → raw rows. Aura-connected. **Live deployment:** **[doggraph.getbarkley.com](https://doggraph.getbarkley.com/)** — the code is the contents of this folder, the AuraDB is real, the secrets sit in Streamlit Cloud's secret store. Nothing is mocked.
...
Skip the setup and go straight to the live demo: **[doggraph.getbarkley.com](https://doggraph.getbarkley.com/)**. Same code as this folder, real AuraDB instance, schema-constrained GraphRAG enabled.
...
- **Live DogGraph app (Streamlit, GraphRAG):** https://doggraph.getbarkley.com/
